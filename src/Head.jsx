      ...
      <div className="flex items-center justify-between px-4 py-2">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2 mt-3">
          <div className={`flex items-center border ${isDark ? 'border-gray-600' : 'border-gray-300'} rounded-full overflow-hidden w-[35vw]`}>
            ...
            />
            <button className={`px-4 py-1 border-l ${isDark ? 'bg-[#2d2d2d] border-gray-600' : 'bg-gray-100 border-gray-300'}`}>
              <img src={searchIcon} alt="search" className={`w-7 h-7 ${isDark ? 'invert' : ''}`} />
            </button>
          </div>
          <div className={`p-2 rounded-full cursor-pointer ${isDark ? 'bg-[#2d2d2d] hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`}>
            <img src={mic} alt="mic" className={`w-7 h-7 ${isDark ? 'invert' : ''}`} />
          </div>
        </div>

        <div className="flex items-center gap-2 mt-2">
        <button onClick={() => setIsDark(!isDark)} className={`text-sm font-bold px-2 ${isDark ? 'text-white' : 'text-black'} rounded-lg transition-colors`}>
          {isDark ? 'Dark' : 'Light'}
        </button>
          <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center cursor-pointer overflow-hidden">
